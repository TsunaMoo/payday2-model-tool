...
namespace PD2ModelParser.Exporters
{
    class GltfExporter
    {
        ...
        void CreateNodeFromObject3D(Object3D thing, GLTF.IVisualNodeContainer parent)
        {
            ...
            Log.Default.Warn(
                "EXPORT OBJECT: Name={0}, Type={1}, SectionId={2}, Parent={3}, Children={4}",
                thing.Name,
                thing.GetType().FullName,
                thing.SectionId,
                thing.Parent?.Name ?? "<null>",
                thing.children.Count);
            ...
            if (thing is Model mod)
            {
                if (mod.version == 3)
                {
                    // Normal mesh-backed Model.
                    //
                    // co_box is one of these. Despite having "box" in its
                    // name, it has PassthroughGP/TopologyIP and therefore
                    // uses the normal mesh path.
                ...
                else if (mod.version == 6)
                {
                    if (IsPrimitiveModel(mod))
                    {
                        ...
                        Log.Default.Warn(
                            "EXPORT PRIMITIVE: Name={0}, BoundsMin={1}, BoundsMax={2}, Unknown7={3}",
                            mod.Name,
                            mod.BoundsMin,
                            mod.BoundsMax,
                            mod.v6_unknown7);
                ...
                {
                    throw new Exception(
                        $"Model {mod.Name} is of unknown version {mod.version}");
                }
            }
            else if (thing is Light dl)
            {
                ...
                {
                    case 1:
                        lamp = root.CreatePunctualLight(
                            GLTF.PunctualLightType.Point);
                        break;

                    case 2:
                        lamp = root.CreatePunctualLight(
                            GLTF.PunctualLightType.Spot);
                        break;

                    default:
                        throw new Exception(
                            $"{thing.Name} is an unknown light type {dl.LightType}");
                }

                lamp.Color = new Vector3(
                    dl.Colour.R,
                    dl.Colour.G,
                    dl.Colour.B);
        ...
        void SkinModel(Model model, GLTF.Node node)
        {
            ...
            foreach (var bone in skeletonObjects)
            {
                ...
                if (MathF.Abs(ibm.M14) < matrixEpsilon)
                    ibm.M14 = 0;

                if (MathF.Abs(ibm.M24) < matrixEpsilon)
                    ibm.M24 = 0;

                if (MathF.Abs(ibm.M34) < matrixEpsilon)
                    ibm.M34 = 0;

                if (MathF.Abs(ibm.M44 - 1.0f) < matrixEpsilon)
                    ibm.M44 = 1.0f;
        ...
        bool IsPrimitiveModel(Model model)
        {
            if (model == null)
                return false;

            // The generated primitive Models we have observed are v6
            // and have no normal render geometry.
            if (model.version != 6)
                return false;
        ...
        GLTF.Mesh CreatePrimitiveMesh(Model model)
        {
            ...
            if (name.StartsWith("c_sphere_", StringComparison.OrdinalIgnoreCase))
                return CreateSphereMesh(model);

            if (name.StartsWith("c_capsule_", StringComparison.OrdinalIgnoreCase))
                return CreateCapsuleMesh(model);

            if (name.StartsWith("c_box_", StringComparison.OrdinalIgnoreCase))
                return CreateBoxMesh(model);
            ...
            var vertices = new List<Vector3>
    {
        ...
        GLTF.Mesh CreateCapsuleMesh(Model model)
        {
            ...
            // The observed capsule models are elongated along one local axis.
            // Determine that axis from the bounds rather than assuming Z.
            int axis;
            ...
            float radius =
                axis == 0
                    ? MathF.Min(size.Y, size.Z) * 0.5f
                    : axis == 1
                        ? MathF.Min(size.X, size.Z) * 0.5f
                        : MathF.Min(size.X, size.Y) * 0.5f;

            float cylinderLength =
                MathF.Max(0, largest - radius * 2.0f);
            ...
            // Build the capsule internally along Z.
            //
            // It is then rotated into whichever axis the source bounds
            // indicate is the capsule's long axis.
            ...
            // Rotate the internally-Z-oriented capsule into the
            // axis represented by the source bounds.
            if (axis != 2)
            {
            ...
            ushort[] indices)
        {
            ...
            var positionAccessor = MakeVertexAttributeAccessor(
                $"{name}_position",
                vertices,
                12,
                GLTF.DimensionType.VEC3,
                i => i,
                ma => ma.AsVector3Array());

            var indexAccessor = CreateIndexAccessor(
                $"{name}_indices",
                indices);
        ...
        {
            var buf = new ArraySegment<byte>(
                new byte[indices.Length * sizeof(ushort)]);

            var mai = new MemoryAccessInfo(
                name,
                0,
                indices.Length,
                0,
                GLTF.DimensionType.SCALAR,
                GLTF.EncodingType.UNSIGNED_SHORT);
        ...
        GLTF.Mesh GetMeshForModel(Model model)
        {
            ...
            if (model.SkinBones != null)
            {
                ...
                for (int i = 0; i < skeletonObjects.Count; i++)
                    jointIndices[skeletonObjects[i]] = i;
        ...
        List<(string, GLTF.Accessor)> GetGeometryAttributes(Geometry geometry, Dictionary<int, int> jointRemap)
        {
            ...
            {
                Func<GeometryWeightGroups, Vector4> ConvertWeightGroup = (groups) =>
                {
                    int Remap(ushort boneIndex)
                    {
                        if (jointRemap.TryGetValue(boneIndex, out var jointIndex))
                            return jointIndex;
                ...
                var a_joint = MakeVertexAttributeAccessor(
                    "vjoint",
                    geometry.weight_groups,
                    8,
                    GLTF.DimensionType.VEC4,
                    ConvertWeightGroup,
                    ma => ma.AsVector4Array(),
                    GLTF.EncodingType.UNSIGNED_SHORT);
